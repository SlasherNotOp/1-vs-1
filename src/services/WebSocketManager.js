...
class WebSocketManager {
  constructor(wss) {
    ...
    this.matchmaking = new MatchmakingSystem(new CodingProblemService(prisma));
    ...
    this.wss.on("connection", (ws, req) => {
      ...
      ws.on("message", async (data) => {
        ...
        switch (event) {
          ...
          case "check_code":
            return this.checkStatusAndUpdateDB(ws, payload);
          default:
            return ws.send(
  ...
  async handleAuth(ws, { token }) {
    try {
      ...
      ws.send(
        JSON.stringify({ event: "authentication_error", message: e.message })
      );
    }
  }
  ...
  handleFindMatch(ws) {
    if (!ws.userId)
      return ws.send(
        JSON.stringify({ event: "error", message: "Not authenticated" })
      );
    const player = {
      id: ws.userId,
      username: ws.username,
      eloRating: ws.eloRating,
    };
    ws.send(
      JSON.stringify({ event: "matchmaking_status", status: "searching" })
    );
  ...
  handleCancel(ws) {
    if (ws.userId) {
      this.matchmaking.removeFromQueue(ws.userId);
      ws.send(
        JSON.stringify({ event: "matchmaking_status", status: "cancelled" })
      );
    }
  }
  ...
  async handleCancelMatch(ws, { matchId }) {
    ...
    if (!match) {
      return ws.send(
        JSON.stringify({
          event: "error",
          message: "Match not found or already canceled",
        })
      );
    }

    // Confirm user is in the match
    const isPlayerInMatch = [match.player1.id, match.player2.id].includes(
      ws.userId
    );
    if (!isPlayerInMatch) {
      return ws.send(
  ...
  async handleSubmit(ws, { matchId, code, language }) {
    ...
    if (!match)
      return ws.send(
        JSON.stringify({ event: "error", message: "Match not found" })
      );
    if (![match.player1.id, match.player2.id].includes(ws.userId)) {
      return ws.send(
        JSON.stringify({ event: "error", message: "Not in this match" })
      );
    }
    if (match.winner) {
      return ws.send(
        JSON.stringify({ event: "error", message: "Match already finished" })
      );
    }

    ws.send(
      JSON.stringify({ event: "submission_status", status: "processing" })
    );
    const testCases = await prisma.testCase.findMany({
      where: { problemId: match.problem.id },
    });
    console.log(testCases, "testCases");
    const tokens = await this.judgeService.submitCode(
      code,
      language,
      testCases
    );
  ...
  async checkStatusAndUpdateDB(ws, { submissionId }) {
    if (!ws.userId)
      return ws.send(
        JSON.stringify({ event: "error", message: "Not authenticated" })
      );
    ...
    const results = await Promise.all(
      tokens.map(async (token) => {
        const result = await this.judgeService.getSubmissionResult(token.id); // you implement this
        await prisma.token.update({
          where: { id: token.id },
          data: { status: result.data.status.description },
        });
        return result;
      })
    );
    ...
    return {
      allTestsPassed: allPassed,
      passedTests: results.filter(r => r.status === 'Accepted').length,
      totalTests: results.length
    };
  }
